module SearchHelper
  IGNORE_WORDS = 
    # List of stop words from http://www.lextek.com/manuals/onix/stopwords1.html
    %w(
      ...
      into  is  it  its  itself  j  just  k  keep  keeps  kind  knew  know  known
      knows  l  large  largely  last  later  latest  least  less  let  lets  like
      likely  long  longer  longest  m  made  make  making  man  many  may  me
      member  members  men  might  more  most  mostly  mr  mrs  much  must  my
      myself  n  necessary  need  needed  needing  needs  never  new  new  newer
      newest  next  no  nobody  non  noone  not  nothing  now  nowhere  number
      numbers  o  of  off  often  old  older  oldest  on  once  one  only  open
      opened  opening  opens  or  order  ordered  ordering  orders  other  others
      our  out  over  p  part  parted  parting  parts  per  perhaps  place  places
      ...
      v  very  w  want  wanted  wanting  wants  was  way  ways  we  well  wells
      went  were  what  when  where  whether  which  while  who  whole  whose  why
  ...
  def search_index(items)
    Hash[items.map do |item|
      words = decode_html_entities(strip_html(item.compiled_content))
        .downcase
        .split(/\W+/)
        .uniq - IGNORE_WORDS
      [item.path, { id: item.path, title: item[:title], tags: (item[:tags] || []).join(","), body: words.join(" ") }]
    end]
  end
end
